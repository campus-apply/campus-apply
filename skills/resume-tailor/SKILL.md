---
name: resume-tailor
description: 对照一份 JD，从事实库挑素材、定口径、写出结构化定稿文字，在用户现有 docx 上原地改出一页简历，并生成网申长文本。Use when the user has a target job (JD) and wants to tailor their resume or write application-form essays; triggers: 改简历 / 针对这个岗位 / 写网申内容 / tailor resume.
---
...
7. 如果这站有网申长文本：先让 apply-fill 做入口探测和上传试探（站点会自动解析简历时，先看解析填到了哪里、剩下哪些要写），再按 `templates/form.template.md` 写 `form.md`，字段名和上限以 `fill-log.md` 里的探测结果为准；与页面无关的内容（岗位要求的自述、证明材料段落）可以先起草给用户过目。写完跑 `check_content.py form.md --limits limits.json`。
