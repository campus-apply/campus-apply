...
**Codex**：终端里 `codex plugin marketplace add campus-apply/campus-apply`，再 `codex plugin add campus-apply@campus-apply`（它认 `.claude-plugin/` 清单；也可以在会话里输 `/plugins` 用界面装），装到 `~/.codex/plugins/cache/campus-apply/campus-apply/<版本>/`。更新：`codex plugin marketplace upgrade campus-apply` 再 `codex plugin add campus-apply@campus-apply`，装好的版本不会自己变。不用插件的话，clone 仓库后运行 `./install.sh codex`，装到 `$CODEX_HOME/skills`（默认 `~/.codex/skills`；主目录不是 `~/.codex` 就先 `export CODEX_HOME=<主目录>`），更新时重新拉仓库再跑一遍脚本。它的沙箱默认不开网络、连不上本机调试端口，要在 `config.toml`（`$CODEX_HOME/config.toml`）的 `[sandbox_workspace_write]` 下加一行 `network_access = true`。

**DeepSeek Harness**（`npm install -g @deepseek-ai/dsh` 后命令是 `dsh`）：clone 仓库后运行 `./install.sh agents`，装到它读取的 `~/.agents/skills`；也可以把 `skills/` 下的五个目录复制进工作目录的 `.agents/skills/`，只在这个工作目录里可见。它没有 marketplace，"让 agent 装"就是让它 clone 仓库再跑这个脚本；它默认只能写当前工作目录，写 `~/.agents/skills` 时会请求放开权限，批准即可，不批准它会装到工作目录里。更新时重新拉仓库再跑一遍脚本。
